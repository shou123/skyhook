...
class SplittedParquetWriter(object):
    def __init__(self, filename, destination, chunksize=128*1024*1024):
        ...
        # print(f'filename: {filename}, destination: {destination}, chunksize: {chunksize}')
    ...
    def write(self):
        ...
        print(f'total_rows: {total_rows}, rows_per_file: {rows_per_file}')
        i = 0
        with ThreadPoolExecutor(max_workers=mp.cpu_count()) as executor:
            print(f'max_workers: {mp.cpu_count()}')
            while i < total_rows:
                executor.submit(
                    self.write_file,
                    os.path.join(
...
if __name__ == "__main__":
    chunksize = 16 * 1024 * 1024 # 13MB
    dataset_dir = str(sys.argv[1])

    # iterate through all directories and files in the directory tree
